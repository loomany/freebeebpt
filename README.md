...
4. Перед AI-обработкой бот проверяет антидубль по `article_hash`, `normalized_title` и похожим заголовкам в SQLite: в БД сохраняются `article_hash`, `url`, `status`, `sent_to_channel`; после рестарта эти записи повторно читаются из БД.
