...
- Если в TEAM_OR_PLAYER_NAMES есть спортсмены или команды, обязательно опирайся на них в image_prompt_en.
- Image prompt должен точно совпадать с сюжетом новости: пол спортсмена, вид спорта, форма, команда, возрастная категория и контекст события не должны противоречить статье.
- Не делай generic image. Если новость про конкретного спортсмена, укажи его имя в image_prompt_en вместе с кратким описанием действия/сцены из новости.
...
def build_news_user_prompt(payload: dict[str, Any]) -> str:
    return (
        ...
        "   - создай image_prompt_en для генерации сильной спортивной news-картинки, которая строго совпадает с сюжетом новости\n"
